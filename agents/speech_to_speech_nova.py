# -*- coding: utf-8 -*-
"""
Speech-to-Speech Agent for CityPulse
Complete pipeline: Audio Input → Transcription → AI Response → Audio Output
...
class SpeechToSpeechAgent:
    ...
    def __init__(self):
        print("="*70)
        print("  SPEECH-TO-SPEECH AGENT - CityPulse")
        print("="*70)
    ...
    def transcribe_audio(self, audio_file_path: str) -> str:
        ...
        job_name = f"citypulse-{int(time.time())}"
        ...
        try:
            ...
            self.transcribe.start_transcription_job(
                ...
                MediaFormat=file_name.split('.')[-1].lower(),
                LanguageCode='en-US'
            )
...
Provide a clear, concise answer in 2-3 sentences. Be conversational and helpful."""
    ...
    def process_speech_to_speech(self, input_audio_path: str) -> dict:
        """
        ...
        try:
            ...
            save_json_data('speech_to_speech_result.json', result)
            
            # Log cost
            log_cost(
                agent_name="speech_to_speech",
            ...
            print("="*70)
            print("  ✅ SPEECH-TO-SPEECH COMPLETE")
            print("="*70)
            print(f"\n📊 Summary:")
            print(f"  Question: \"{question}\"")
            print(f"  Answer: \"{answer}\"")
...
def main():
    """Test the speech-to-speech agent"""
    ...
    if not os.path.exists(test_audio):
        ...
        print(f"Please provide an audio file named '{test_audio}' in the current directory")
        print("\nSupported formats: MP3, MP4, WAV, FLAC, OGG, AMR, WebM")
        print("\nExample questions to record:")
        print("  - 'What are the trending topics in Mumbai?'")
        print("  - 'Are there any safety alerts?'")
        print("  - 'Which neighborhoods are good for investment?'")
        print()
        return
    
    try:
        agent = SpeechToSpeechAgent()
        result = agent.process_speech_to_speech(test_audio)
        
        if result:
            print("✅ Test completed successfully!")
            print(f"📁 Result saved to: data/speech_to_speech_result.json")
